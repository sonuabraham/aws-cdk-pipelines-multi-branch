"""
Lambda function code used to create a CodeBuild project which deploys the CDK pipeline stack for the branch.
"""
import logging
import os

import boto3

logger = logging.getLogger()
logger.setLevel(logging.INFO)

client = boto3.client('codebuild')
region = os.environ['AWS_REGION']
account_id = os.environ['ACCOUNT_ID']
role_arn = os.environ['CODE_BUILD_ROLE_ARN']
artifact_bucket_name = os.environ['ARTIFACT_BUCKET']
codebuild_name_prefix = os.environ['CODEBUILD_NAME_PREFIX']


def generate_build_spec(branch: str):
    """Generates the build spec file used for the CodeBuild project"""
    return f"""version: 0.2
env:
  variables:
    BRANCH: {branch}
    DEV_ACCOUNT_ID: {account_id}
    PROD_ACCOUNT_ID: {account_id}
    REGION: {region}
phases:
  pre_build:
    commands:
      - npm install -g aws-cdk && pip install -r requirements.txt
  build:
    commands:
      - cdk synth
      - cdk deploy --require-approval=never
artifacts:
  files:
    - '**/*'"""


def handler(event, context):
    """
    Lambda function handler for branch creation events.
    Supports both CodeCommit and GitHub event formats.
    """
    logger.info(f"Received event: {event}")

    try:
        detail = event.get('detail', {})
        reference_type = detail.get('referenceType')

        if reference_type == 'branch':
            branch = detail.get('referenceName')
            repo_name = detail.get('repositoryName')
            
            if not branch or not repo_name:
                logger.error(f"Missing required fields: branch={branch}, repo_name={repo_name}")
                return
            
            # Detect if this is a GitHub event (repository name contains '/')
            is_github = '/' in repo_name
            
            logger.info(f"Processing branch creation: branch={branch}, repo={repo_name}, is_github={is_github}")
            
            if is_github:
                # GitHub repository format: owner/repo
                source_config = {
                    'type': 'GITHUB',
                    'location': f'https://github.com/{repo_name}.git',
                    'buildspec': generate_build_spec(branch)
                }
            else:
                # CodeCommit repository
                source_config = {
                    'type': 'CODECOMMIT',
                    'location': f'https://git-codecommit.{region}.amazonaws.com/v1/repos/{repo_name}',
                    'buildspec': generate_build_spec(branch)
                }

            project_name = f'{codebuild_name_prefix}-{branch}-create'
            
            logger.info(f"Creating CodeBuild project: {project_name}")
            
            client.create_project(
                name=project_name,
                description="Build project to deploy branch pipeline",
                source=source_config,
                sourceVersion=f'refs/heads/{branch}',
                artifacts={
                    'type': 'S3',
                    'location': artifact_bucket_name,
                    'path': f'{branch}',
                    'packaging': 'NONE',
                    'artifactIdentifier': 'BranchBuildArtifact'
                },
                environment={
                    'type': 'LINUX_CONTAINER',
                    'image': 'aws/codebuild/standard:6.0',
                    'computeType': 'BUILD_GENERAL1_SMALL'
                },
                serviceRole=role_arn
            )

            logger.info(f"Starting CodeBuild project: {project_name}")
            
            client.start_build(
                projectName=project_name
            )
            
            logger.info(f"Successfully created and started build for branch: {branch}")
            
    except Exception as e:
        logger.error(f"Error processing branch creation: {e}", exc_info=True)
        raise
